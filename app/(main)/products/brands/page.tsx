...
export default function BrandsPage() {
  ...
  const handleOpenDelete = (item: Brand) => {
    setDeleteItem(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem
        ? `/api/products/brands/${editingItem.id}`
        : "/api/products/brands";
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/products/brands/${deleteItem.id}`, {
        method: "DELETE",
      });
        ...
        <CardHeader className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white !px-6 !py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-2xl font-bold">แบรนด์สินค้า</CardTitle>
            </div>
            {canCreate && (
              <Button
        ...
        <CardContent className="p-6">
          ...
          {total > 0 && (
            <div className="mt-4 text-sm text-slate-500">
              แสดง {brands.length} จาก {total} รายการ
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>กรอกข้อมูลแบรนด์ด้านล่าง</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="code">รหัสแบรนด์ *</Label>
              <Input
                id="code"
                value={formData.code}
                onChange={(e) =>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              คุณต้องการลบแบรนด์ <strong>{deleteItem?.description}</strong>{" "}
              ใช่หรือไม่?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
