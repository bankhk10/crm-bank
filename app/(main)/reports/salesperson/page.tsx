      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              เจาะลึกผลงานรายบุคคลและกลุ่มสินค้า
            </p>
          </div>
        </div>
        ...
        <Card className="rounded-2xl border bg-white/80 shadow-sm overflow-hidden">
          <CardContent className="p-4">
            ...
            <div
              className={`mt-4 space-y-4 sm:space-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:gap-4 ${filtersOpen ? "block" : "hidden"} sm:block`}
            >
        ...
        {isPending ? (
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard
                title="พนักงานขายดีที่สุด"
                value={reportData.topSalesperson?.name || "-"}
                subValue={formatTHB(reportData.topSalesperson?.sales || 0)}
                icon={<Award className="text-rose-600" />}
                color="bg-rose-50"
              />
              <StatCard
                title="ยอดขายรวม"
                value={formatTHB(totalSales)}
              ...
              <StatCard
                ...
                value={formatNumber(reportData.salespersonPerformance.length)}
            ...
            >
              <TabsList className="grid grid-cols-2 lg:flex h-auto p-1 bg-slate-100 rounded-xl">
                <TabsTrigger
                  value="performance"
                  className="data-[state=active]:bg-white rounded-lg py-2"
                >
                  ผลงาน
                </TabsTrigger>
              </TabsList>
          ...
          <Card className="rounded-3xl border-dashed border-2 bg-transparent">
            <CardContent className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-slate-400 max-w-xs mt-2">
                กรุณาเลือกช่วงวันที่คุณต้องการตรวจสอบข้อมูล แล้วกดปุ่ม
                "ดูรายงาน"
              </p>
            </CardContent>
          </Card>
    ...
    <Card className="rounded-2xl border shadow-sm">
      <CardContent className="p-5 flex justify-between items-start">
        <div className="space-y-1">
          ...
          {subValue && (
            <p className="text-sm font-medium text-rose-500">{subValue}</p>
          )}
        </div>
