      ...
      <div className="p-6 space-y-3">
        {stockWarnings.map((w, i) => (
          <div key={i} className="flex flex-wrap items-center gap-x-2 text-sm">
            ...
            <span className="font-bold text-green-600">{w.requested}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 shadow-sm text-center">
          ...
          <p
            className={`font-bold text-xl mt-1 ${
              creditInfo.willExceedLimit ? "text-red-600" : "text-emerald-600"
            }`}
          >
                          ...
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {item.lotAllocations.map(
                              (alloc: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="text-[10px] text-gray-600 bg-purple-50 px-2 py-1 rounded-md inline-flex items-center gap-1 border border-purple-100"
                                >
                                  <span className="font-bold text-purple-700">
                                    LOT: {alloc.lot?.lotNumber || "-"}
                                  </span>
                    ...
                    {Number(item.promotionBudget ?? 0) > 0 && (
                      <div className="col-span-2 bg-emerald-50/50 rounded-lg p-3 text-right border border-emerald-100 mt-1">
                        ...
                        <p className="font-bold text-emerald-600">
                          ฿
                          {(
                            Number(item.quantity) * Number(item.promotionBudget)
              ...
              return (
                ...
                >
                  <div className="grid grid-cols-12 gap-4 items-center p-4 md:p-5">
                            ...
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              {item.lotAllocations.map(
                                (alloc: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="text-[10px] text-gray-600 bg-purple-50 px-2 py-0.5 rounded-md inline-flex items-center gap-1.5 border border-purple-100"
                                  >
                                    <span className="font-bold text-purple-700">
                                      LOT: {alloc.lot?.lotNumber || "-"}
                                    </span>
                    ...
                    <div className="col-span-1 text-right">
                      <span
                        className={`font-bold text-base ${priceChanged ? "text-orange-700" : "text-blue-600"}`}
                      >
                        ฿
                        {currentTotal.toLocaleString("th-TH", {
                          minimumFractionDigits: 2,
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200">
                    <div className="flex justify-between items-end bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                      <div className="flex flex-col">
                        <span className="text-xl uppercase text-gray-600 font-black mb-1">
                          ยอดสุทธิ
                        </span>
                      </div>
