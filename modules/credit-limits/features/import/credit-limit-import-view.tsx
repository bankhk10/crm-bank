...
import {
  importCreditLimitsAction,
  previewCreditLimitsAction,
  downloadCreditLimitTemplateAction,
} from "../../server/actions";
import type { ImportPreviewRow } from "../../application/import-credit-limits";

type Step = "upload" | "preview" | "result";

export default function CreditLimitImportView() {
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      const res = await downloadCreditLimitTemplateAction();
      if (res.success && res.data) {
        ...
        a.href = url;
        a.download = "template_credit_limits.xlsx";
        a.click();
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const res: any = await previewCreditLimitsAction(formData);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importCreditLimitsAction(formData);
      setResult(res);
      setStep("result");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                อัปโหลดไฟล์ Excel เพื่ออัพเดทวงเงินเครดิตลูกค้าหลายรายการพร้อมกัน
              </p>
            </div>
          </div>
        </div>
            ...
            <Card className="border-slate-100">
              ...
              <CardContent className="pb-5">
                <ul className="text-sm text-slate-600 list-disc list-inside space-y-2">
                  <li><strong>รหัสลูกค้า</strong> เปลี่ยนแปลงไม่ได้ ต้องมีอยู่จริงในระบบ</li>
                  <li>สามารถเว้นว่างคอลัมน์ที่ไม่ต้องการอัพเดทได้</li>
                  <li>ข้อมูลวงเงินเครดิตจะถูกบันทึกทับข้อมูลเดิมทันที</li>
                  <li>วงเงินส่งเสริมการขายจะถูกบันทึกในปีปัจจุบัน ถ้ามีอยู่แล้วยอดโปรโมชั่นจะถูกปรับปรุง</li>
                </ul>
              </CardContent>
            </Card>
                      ...
                      {previewData.slice(0, 50).map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-right text-slate-700 tabular-nums">
                            {row.limitAmount ? row.limitAmount.toLocaleString("th-TH") : "-"}
                          </td>
                          <td className="px-3 py-2 text-right text-slate-700 tabular-nums">
                            {row.promoAmount ? row.promoAmount.toLocaleString("th-TH") : "-"}
                          </td>
                          <td className="px-3 py-2 text-right text-slate-700 tabular-nums">
                            {row.temporaryCreditAmount ? row.temporaryCreditAmount.toLocaleString("th-TH") : "-"}
                          </td>
                          <td className="px-3 py-2 text-slate-600 text-xs whitespace-nowrap">
                            {row.temporaryCreditExpiryDate}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Card className="border-emerald-100">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-sm text-slate-500 mt-1">อัพเดทสำเร็จ</p>
                  </CardContent>
                </Card>
