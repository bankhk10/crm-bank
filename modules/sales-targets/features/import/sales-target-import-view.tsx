...
import {
  ...
  previewSalesTargetsAction,
  downloadSalesTargetTemplateAction,
} from "../../server/import-actions";
import type { ImportPreviewRow } from "../../application/import-sales-targets";
...
export default function SalesTargetImportView() {
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      const res = await downloadSalesTargetTemplateAction();
      if (res.success && res.data) {
        ...
        a.href = url;
        a.download = "template_sales_target.xlsx";
        a.click();
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const res = await previewSalesTargetsAction(formData);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await importSalesTargetsAction(formData);
      setResult(res);
      setStep("result");
  ...
  const getMonthLabel = (month: number) =>
    MONTHS.find((m) => m.value === month)?.label ?? `เดือน ${month}`;
      ...
      <div className="p-4 sm:p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-800">
                นำเข้าเป้าหมายการขาย
              </h1>
              <p className="text-sm text-slate-500 mt-0.5">
                อัปโหลดไฟล์ Excel เพื่อเพิ่มหรืออัปเดตเป้าหมายการขายเป็นกลุ่ม
              </p>
            </div>
          </div>
        </div>
          ...
          {["อัปโหลดไฟล์", "ตรวจสอบข้อมูล", "ผลลัพธ์"].map((label, idx) => {
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                {idx > 0 && (
                  <div
                    className={`hidden sm:block w-8 h-0.5 ${isPast ? "bg-emerald-500" : "bg-slate-200"}`}
                  />
                )}
        ...
        {step === "upload" && (
          <div className="max-w-3xl mx-auto space-y-6">
            {/* Template Download */}
            <Card className="border-dashed border-emerald-200 bg-gradient-to-br from-emerald-50/50 to-teal-50/50">
              ...
              <CardContent className="pb-5">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {[
                    ...
                    { name: "จำนวน (ลัง)", desc: "จำนวนเต็ม" },
                    { name: "ราคา/ลัง", desc: "บาท" },
                    { name: "เป้าหมาย (บาท)", desc: "ยอดรวม" },
                  ].map((col) => (
                    <div
                      key={col.name}
                      ...
                      {previewData.slice(0, 50).map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-slate-700">
                            {row.year > 0 ? `${row.year + 543}` : "-"}/
                            {row.month > 0 ? getMonthLabel(row.month) : "-"}
                          </td>
                          ...
                          <td className="px-3 py-2 text-right text-slate-700 tabular-nums">
                            {row.qtyPerBox.toLocaleString()}
                          </td>
                          <td className="px-3 py-2 text-right text-slate-700 tabular-nums">
                            {row.pricePerBox.toLocaleString("th-TH", {
                              minimumFractionDigits: 2,
                            })}
                          </td>
                          <td className="px-3 py-2 text-right font-medium text-slate-800 tabular-nums">
                            {row.targetAmount.toLocaleString("th-TH", {
                              minimumFractionDigits: 2,
                            })}
                          </td>
                        </tr>
                ...
                <Card className="border-emerald-100">
                  <CardContent className="p-4 text-center">
                    <p className="text-3xl font-bold text-emerald-600">
                      {result.importedTargets || 0}
                    </p>
                    <p className="text-sm text-slate-500 mt-1">สร้างใหม่</p>
                  </CardContent>
                </Card>
                <Card className="border-blue-100">
                  <CardContent className="p-4 text-center">
                    <p className="text-3xl font-bold text-blue-600">
                      {result.updatedTargets || 0}
                    </p>
                    <p className="text-sm text-slate-500 mt-1">อัปเดต</p>
                  </CardContent>
                </Card>
              </div>
            )}
