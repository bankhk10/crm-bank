...
import type { ActivityPlanWithRelations, ActivityStatus, ActivityHelperStatus, ActivityApprovalStep } from "../../types";
import {
...
export default function ActivityPlanDetailView({ id }: Props) {
  ...
  const salesPromoVal = plan.salesPromotionBudget ? Number(plan.salesPromotionBudget) : 0;
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm space-y-6">
            <div className="flex justify-between items-start gap-4">
              <div>
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">ชื่อแผนงานกิจกรรม</span>
                <h2 className="text-xl font-bold text-slate-800 mt-0.5">{plan.title}</h2>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <DetailItem label="ผู้จัดทำ" value={`${plan.employee.name} (${plan.employee.positionTitle || "ไม่ระบุตำแหน่ง"})`} icon={<User className="h-4 w-4" />} />
              <DetailItem label="ประเภทกิจกรรม" value={plan.activityType} icon={<FileText className="h-4 w-4" />} />
              <DetailItem
              ...
              />
              <DetailItem label="สถานที่ / พื้นที่จัดกิจกรรม" value={plan.location} icon={<MapPin className="h-4 w-4" />} />
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1 whitespace-pre-line leading-relaxed">{plan.objective}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1 whitespace-pre-line leading-relaxed">{plan.description}</p>
              </div>

              {plan.notes && (
                <div>
                  <h4 className="text-sm font-bold text-slate-700">หมายเหตุเพิ่มเติม</h4>
                  <p className="text-sm text-slate-500 mt-1 whitespace-pre-line italic">"{plan.notes}"</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm space-y-4">
            ...
            {budgetTotal === 0 ? (
              <p className="text-sm text-slate-400 italic">ไม่มีความจำเป็นต้องใช้วงเงินงบประมาณในกิจกรรมนี้</p>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                <div className={cn("p-4 rounded-xl border border-dashed flex justify-between items-center", salesPromoVal > 0 ? "bg-blue-50/50 border-blue-100" : "bg-slate-50/50 border-slate-100")}>
                  <div>
                    <span className="text-xs text-slate-400 block font-medium">งบส่งเสริมการขาย</span>
                    <span className="text-lg font-bold text-slate-800 mt-1 block">
                      {new Intl.NumberFormat("th-TH", { style: "currency", currency: "THB", maximumFractionDigits: 0 }).format(salesPromoVal)}
                    </span>
                  </div>
                  {salesPromoVal > 0 && (
                    <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full", plan.salesPromotionApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700")}>
                      {plan.salesPromotionApproved ? "อนุมัติแล้ว" : "รออนุมัติ"}
                    </span>
                  )}
                </div>

                <div className={cn("p-4 rounded-xl border border-dashed flex justify-between items-center", marketingVal > 0 ? "bg-purple-50/50 border-purple-100" : "bg-slate-50/50 border-slate-100")}>
                  <div>
                    <span className="text-xs text-slate-400 block font-medium">งบการตลาด</span>
                    <span className="text-lg font-bold text-slate-800 mt-1 block">
                      {new Intl.NumberFormat("th-TH", { style: "currency", currency: "THB", maximumFractionDigits: 0 }).format(marketingVal)}
                    </span>
                  </div>
                  {marketingVal > 0 && (
                    <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full", plan.marketingApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700")}>
                ...
                {plan.status === "PENDING_BUDGET_APPROVAL" && (
                  <div className="md:col-span-2 bg-blue-50/50 p-3 rounded-lg border border-blue-100 flex items-center gap-2 text-xs text-blue-700">
                    ...
                    <span>
                      สถานะงบประมาณส่วนกลาง: <strong>{plan.salesManagerApproved ? "อนุมัติผ่านงบฝ่ายขายแล้ว" : "รอผู้จัดการฝ่ายขายอนุมัติปิดท้ายงบประมาณภาพรวม"}</strong>
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm space-y-4">
            ...
            {plan.helpers.length === 0 ? (
              <p className="text-sm text-slate-400 italic">ไม่มีพนักงานช่วยงานเพิ่มเติมในทริปนี้</p>
            ) : (
              <div className="divide-y border rounded-xl overflow-hidden bg-slate-50/30">
                {plan.helpers.map((helper) => (
                  <div key={helper.id} className="p-3.5 flex justify-between items-center text-sm">
                    <div>
                      <span className="font-semibold text-slate-900 block">{helper.employee.name}</span>
                      <span className="text-xs text-slate-500 mt-0.5 block">
                        ตำแหน่ง: {helper.employee.positionTitle || "ไม่ระบุ"} | แผนก: {helper.employee.departmentName || "ไม่ระบุ"}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span
                        className={cn(
                          ...
                          helper.status === "APPROVED" && "bg-green-100 text-green-700",
                          helper.status === "PENDING" && "bg-yellow-100 text-yellow-700",
                      ...
                      {helper.rejectionReason && (
                        <span className="text-xs text-red-500 font-medium max-w-[150px] truncate" title={helper.rejectionReason}>
          ...
          {plan.createdById === session?.user?.id &&
            ...
            plan.status !== "CANCELLED" && (
              <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                <Button variant="outline" onClick={handleCancelPlan} disabled={submitting} className="w-full text-red-600 border-red-100 hover:bg-red-50 font-semibold">
                  ยกเลิกแผนกิจกรรมนี้
                </Button>
              </div>
            )}
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="relative pl-6 border-l-2 border-slate-100 space-y-6">
              {plan.approvalLogs.length === 0 ? (
                <p className="text-xs text-slate-400 italic">ไม่มีข้อมูลบันทึกในประวัติ</p>
              ) : (
                plan.approvalLogs.map((log) => {
                  ...
                  if (log.action === "REQUEST_CORRECTION") actionText = "ขอข้อมูลเพิ่มเติม/ตีกลับ";
                  ...
                  return (
                    <div key={log.id} className="relative text-xs">
                      {/* Timeline node */}
                      <span className={cn("absolute -left-[31px] top-1 h-3.5 w-3.5 rounded-full border-2 border-white ring-2 ring-slate-100", badgeColor)} />
                      
                      <div className="flex justify-between items-center gap-2">
                        <span className="font-bold text-slate-800">{actionText}</span>
                        <span className="text-[10px] text-slate-400">{format(new Date(log.createdAt), "dd MMM HH:mm", { locale: th })}</span>
                      </div>
